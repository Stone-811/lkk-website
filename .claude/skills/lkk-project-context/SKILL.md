...
description: 練健康官網 lkk-new-web (Vue 3 + Nuxt 3) 的實際架構、部署方式、Firebase/CLI 現況與開發地雷。在此專案開發（apps/newweb 的頁面、Nitro API、/admin 後台、Firestore、部署設定）前先讀，掌握「CLAUDE.md 文件」與「程式碼實況」的落差，避免踩雷。
---
...
## 專案本質
- 練健康（l-kk.tw）官網，`apps/newweb` = Vue 3.5 + Nuxt 3.17（SSR，nitro `firebase-app-hosting`）。monorepo 但實際只有這一個 app。
...
## 部署 / Firebase / CLI
- ⚠️ **遷移中：`lkk-website-dev` → `lkkdev`**（維持 backend 名 newweb、後端 Firestore；lkkdev 有其他既有服務勿動）。目標 projectId=`lkkdev`、bucket=`lkkdev.firebasestorage.app`。已建 lkkdev Firestore@asia-east1 + 索引；`apphosting.yaml` 已指向 lkkdev。待做：建 lkkdev App Hosting backend newweb+連 GitHub、建 secrets、跑 `apps/newweb/scripts/migrate-firestore.mjs` 搬資料。
...
- 本機 `/Users/stone/Downloads/lkk-new-web` 是 **git untracked**（repo 根是家目錄）→ 在這改的不會自動部署，要同步到部署 repo。
...
- 只在根目錄 `npm install`；build 用 `npm run build`（turbo）或 `npm run build:newweb`。
