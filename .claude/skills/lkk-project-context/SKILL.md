...
- Footer（影響每一頁）：分店名 12px／電話 10px → 14px，電話對比 2.90 → 5.51；
  社群圖示 ×6 20×20 → 44×44
- 研習課程 `<summary>` 觸控 20px → 44px；教練職稱移除 `truncate`（11 字被切成 8 字）
- `/booking` 14 處內文 → 14px（含健康聲明條款）
- **`/group-booking` 輸入框 15.2px → 16px** ⚠️ 低於 16px 會讓 **iOS Safari 聚焦時自動
  縮放整頁**，是實際干擾填表的 bug；同頁錯誤訊息 `red-500`(3.76) → `red-600`(4.83)
- 首頁媒體連結：高 20px → 40px、`ink/50`(3.23) → `ink/70`(6.41)

⚠️ **`grep text-xs` 會漏掉一半**——repo 大量使用 `text-[0.82rem]` 這類任意值
（`/group-booking` 有 15 處，最小 10.9px）。兩種都要掃，見 [[lkk-web-gotchas]] 第 34 條。

**業主決定不修**：Footer 文字連結觸控高度 14–24px。
...
**講師（2026-08-28 更新）**：業主兩份「講師自介」CSV 已匯入，練健康 11 位＋海外 1 位＋合作 3 位＝**15 位**。**石峻瑋已依業主指示刪除**（不在 CSV 名單、資料幾乎全空），連同那張戴全罩安全帽的錯誤照片 `liuchang.png` 一併移除，先前的紅字問題已解。仍有 3 位無去背照（李柏橋／吳禎明／阮玟文，是 500x500 白底 PNG 不是去背）。**講師目錄已清乾淨**：4 個中文檔名重複檔（約 2.5MB）於 2026-08-28 依業主指示刪除，現在 15 個檔案對 15 個 API 引用，孤兒 0、缺檔 0。
