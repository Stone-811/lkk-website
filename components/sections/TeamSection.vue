  ...
  <section class="bg-white py-16 lg:py-24">
    <div class="container mx-auto px-4">
      ...
      <h2 class="font-serif text-3xl lg:text-4xl font-black text-navy-700 mb-3">
        由<span class="text-orange">醫療背景</span>專業人員組成
      </h2>
      <p class="text-ink/60 leading-relaxed mb-10">
        練健康成立於 2018 年，由一群關心台灣高齡健康的物理治療師、營養師與專業教練組成，結合最新的醫學研究與運動科學，專注培養專業且具豐富實戰經驗的教練，服務中高齡族群。
      </p>
      ...
      <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        >
          <!-- Avatar -->
          <div class="w-32 h-32 rounded-full bg-white flex items-center justify-center font-serif text-4xl font-bold mb-4 border-[3px] border-cream-200 overflow-hidden relative">
            <img
              v-if="coach.photo"
              :src="coach.photo"
              :alt="coach.name"
            ...
            />
            <span v-else class="absolute inset-0 flex items-center justify-center bg-navy-700 text-white">{{ coach.avatar }}</span>
          </div>

          <!-- Name & role -->
          <h3 class="font-serif text-lg font-bold text-navy-700 mb-1">
            {{ coach.name }} 教練
          </h3>
          <div class="text-sm text-orange font-semibold tracking-wide mb-3">
            {{ coach.role }}
          </div>

          <!-- Description -->
          <p class="text-sm text-ink/50 leading-relaxed mb-4">
            {{ coach.description }}
          </p>
          ...
          <div class="flex flex-wrap gap-1.5 justify-center">
            <span
              v-for="cred in coach.creds"
              :key="cred"
              class="text-xs font-medium text-navy-700 bg-navy-700/[0.08] px-2.5 py-0.5 rounded-full"
            >
              {{ cred }}
            </span>
          </div>
